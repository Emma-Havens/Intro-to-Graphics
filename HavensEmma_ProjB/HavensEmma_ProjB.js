...
var aim_theta = -Math.PI / 2;
var aim_tilt = 0;
var camera_move_rate = 0.03;
var camera_move_rate_side = 0.2;
var camera_swivel_rate = 0.009;
...
function makeGroundGrid() {
	...
	// centered at x=y=z=0.  Draw this shape using the GL_LINES primitive.
		...
		var xymax	= 50.0;			// grid size; extends to cover +/-xymax in x and y.
		 var xColr = new Float32Array([0.5, 0.5, 0.3]);	// bright yellow
		 var yColr = new Float32Array([0.3, 0.5, 0.3]);	// bright green.
...
function initVertexBuffers() {
	  ...
	  makeGroundGrid();							// create, fill gndVerts array
	  makeAxis();								// create, fill axVerts array

	// how many floats total needed to store all shapes?
	var mySiz = (hexVerts.length + d20Verts.length + gndVerts.length + axVerts.length);						
...
function drawScene() {
	pushMatrix(g_modelMatrix);	
		...
		//-------Draw snake ring:
		pushMatrix(g_modelMatrix);
			g_modelMatrix.translate(-2,-0.8, 0.1);  // 'set' means DISCARD old matrix,
		...
		// Draw fractal
		pushMatrix(g_modelMatrix);
			g_modelMatrix.translate(1.0, g_angle1now, 0.2)
		...
		// Draw D20
		pushMatrix(g_modelMatrix);
			g_modelMatrix.translate(-0.6, 0.6, 0.3);
		...
		// Draw centipede
		pushMatrix(g_modelMatrix);
			g_modelMatrix.translate(1, 2, 0.1);
			g_modelMatrix.rotate(-30, 0, 0, 1);
			g_modelMatrix.scale(0.1, 0.1, 0.1);

			var make_body_segment = function() {
				pushMatrix(g_modelMatrix);
				gl.uniformMatrix4fv(uLoc_ModelMatrix, false, g_modelMatrix.elements);
				gl.drawArrays(gl.TRIANGLE_STRIP,				// use this drawing primitive, and
								d20Start/floatsPerVertex, // start at this vertex number, and
				...
				g_modelMatrix.scale(0.2, 0.2, 1.1);
				gl.uniformMatrix4fv(uLoc_ModelMatrix, false, g_modelMatrix.elements);
				gl.drawArrays(gl.TRIANGLE_STRIP,				// use this drawing primitive, and
								hexStart/floatsPerVertex, // start at this vertex number, and
								hexVerts.length/floatsPerVertex);	// draw this many vertices.
			...
			for (num = 0; num < 4; num++) {
				make_body_segment();
				g_modelMatrix.translate(-1.9, 0, 0);
			}
		g_modelMatrix = popMatrix()

		// Draw beetle
		pushMatrix(g_modelMatrix);
			g_modelMatrix.translate(1, -2, 0.15);
			g_modelMatrix.scale(0.15, 0.15, 0.15);
			make_body_segment();
			g_modelMatrix.translate(-2.8, 0, 0);
			pushMatrix(g_modelMatrix);
				g_modelMatrix.scale(2, 1.2, 1.2);
					gl.uniformMatrix4fv(uLoc_ModelMatrix, false, g_modelMatrix.elements);
					gl.drawArrays(gl.TRIANGLE_STRIP,				// use this drawing primitive, and
									d20Start/floatsPerVertex, // start at this vertex number, and
									d20Verts.length/floatsPerVertex);	// draw this many vertices.
			g_modelMatrix = popMatrix();
			pushMatrix(g_modelMatrix);
				g_modelMatrix.translate(0.7, 1, 0);
				g_modelMatrix.rotate(-120, 1, 0, 0);
				g_modelMatrix.translate(0, 0, 0.5);
...
function drawAll() {
  ...
  if (upPressed) {
	aim_tilt += camera_move_rate;
  }	
  if (downPressed) {
	aim_tilt -= camera_move_rate;
  }
  if (leftPressed) {
	aim_theta += camera_swivel_rate;
  }
  if (rightPressed) {
	aim_theta -= camera_swivel_rate;
  }
  if (wPressed) {
	eye_z += camera_move_rate;
  }
  if (sPressed) {
	eye_z -= camera_move_rate;
  }
  if (aPressed) {
	eye_x += Math.cos(aim_theta - Math.PI / 2) * camera_move_rate_side;
	eye_y += Math.sin(aim_theta - Math.PI / 2) * camera_move_rate_side;
  }
  if (dPressed) {
	eye_x += Math.cos(aim_theta + Math.PI / 2) * camera_move_rate_side;
	eye_y += Math.sin(aim_theta + Math.PI / 2) * camera_move_rate_side;
  ...
  if (qPressed) {
	eye_x = eye_x - (aim_x - eye_x) * camera_move_rate;
	eye_y = eye_y - (aim_y - eye_y) * camera_move_rate;
	eye_z = eye_z - (aim_z - eye_z) * camera_move_rate;
  }
  if (ePressed) {
	eye_x = eye_x + (aim_x - eye_x) * camera_move_rate;
	eye_y = eye_y + (aim_y - eye_y) * camera_move_rate;
	...
	var far = 20;
	var fov = 35;
	var aspect = (g_canvas.width / 2) / (3/4 * g_canvas.height);

	// projection matrix
	g_modelMatrix.perspective(	fov,   // FOVY: top-to-bottom vertical image angle, in degrees
	...
	aim_x = eye_x - 5 * Math.cos(aim_theta);
	aim_y = eye_y - 5 * Math.sin(aim_theta);
	aim_z = eye_z + aim_tilt;
	...
	var height = (far - near) / 3 * Math.tan(fov);

	// projection matrix
	g_modelMatrix.ortho(aspect * -height,		// left
						aspect * height,		// right
						-height,		// bottom
						height,	// top
						near,		// near
